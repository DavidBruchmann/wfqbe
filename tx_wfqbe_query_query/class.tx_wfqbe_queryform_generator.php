 ...
 *  Copyright notice
 *
 *  (c) 2006 Menegon Davide (Webformat srl) (menedav@libero.it)
...
class tx_wfqbe_queryform_generator{
	...
	function init(){
		...
		}elseif($this->wfqbefunction=="RAWQUERY" && t3lib_div::_GP('rawwfqbe')==""){//caso in cui passo da QBE a RAWQUERY
			$this->wfqbe=t3lib_div::_GP('wfqbe');
			$this->pass=t3lib_div::_GP('pass');
	...
	function saveQuery(){
		...
		if($this->rawwfqbe['rawquery']!="")	{
			...
			//t3lib_div::debug($this->pass['hiddenraw']);
			$this->rawwfqbe = $API2->xml2array($this->pass['hiddenraw']);
			//t3lib_div::debug($this->rawwfqbe);
			if($this->rawwfqbe['rawquery']!="")	{
				$saveArray['contentwfqbe']['rawwfqbe']['rawquery'] = $this->rawwfqbe['rawquery'];
