...
require_once(t3lib_extMgm::extPath('wfqbe')."lib/class.tx_wfqbe_api_array2xml.php");
...
class tx_wfqbe_insertform_generator{
	...
	* Utilizza la funzione array2xml() definita nella classe tx_wfqbe_api_array2xml che converte un array in un file(stringa) xml.
 	...
 	*/
	function saveModule(){
		$API = t3lib_div::makeInstance("tx_wfqbe_api_array2xml");
		...
		if ($this->blocks['table']!="")	{
			...
			if (is_array($this->blocks['fields']))	{
				foreach($this->blocks['fields'] as $key => $value)	{
					...
					if ($numForm > 0)
						$content.= ' <a href="#" onclick="document.getElementById(\'wfqbe_up\').value='.$numForm.'; updateForm()"><img src="'.t3lib_div::getIndpEnv('TYPO3_SITE_URL').'typo3/sysext/t3skin/icons/gfx/button_up.gif" /></a>';
					if ($numForm < sizeof($this->blocks['fields'])-1)
						$content.= ' <a href="#" onclick="document.getElementById(\'wfqbe_down\').value='.$numForm.'; updateForm()"><img src="'.t3lib_div::getIndpEnv('TYPO3_SITE_URL').'typo3/sysext/t3skin/icons/gfx/button_down.gif" /></a>';
