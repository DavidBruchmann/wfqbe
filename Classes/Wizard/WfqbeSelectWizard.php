...
/**
 * wfqbe module tx_wfqbe_query_querywiz0
 *
 * @author	Davide Menegon <menedav@libero.it>
 * @author  Mauro Lorenzutti <mauro.lorenzutti@webformat.com>
 */
class WfqbeSelectWizard { //  extends \TYPO3\CMS\Backend\Module\BaseScriptClass
			var $P;
			var $qbe;
			/**
 ...
 */
				function main()	{
					...
					$this->id = $this->P['pid'];
					$this->qbe = GeneralUtility::makeInstance("tx_wfqbe_queryform_generator");
					$this->qbe->init();
						// Draw the header.
							...
							function updateForm() {
								$(\'insQuery\').request({
								  ...
								  onComplete: function(xhr, json) {
								        ...
								        document.getElementById(\'tx_wfqbe_queryform\').innerHTML = ""+xhr.responseText;
								    }.bind(this),
					...
					if (($this->id && $access) || ($GLOBALS['BE_USER']->user['admin'] && !$this->id))	{
						...
						//$headerSection = $this->doc->getHeader('pages',$this->pageinfo,$this->pageinfo['_thePath']).'<br>'.$LANG->sL('LLL:EXT:lang/locallang_core.xml:labels.path').': '.GeneralUtility::fixed_lgd_pre($this->pageinfo['_thePath'],50);
						$headerSection = "";
						$this->content.='<div id="tx_wfqbe_queryform" style="padding:0em 3em 0em 3em;">';
							...
							$this->content.=
								...
								$this->doc->section(
									...
									$this->doc->makeShortcutIcon(
										...
										'' //$this->$GLOBALS['TBE_MODULES']['_configuration']['tx_wfqbe_query_query']['name']
									)
								);
						}
						*/
			...
			function moduleContent() {
				...
				$content='';
					//switch((string)$this->MOD_SETTINGS['function'])	{
						//case 1:
								$var=GeneralUtility::_GP('P');//P � l'array che contiene tutte le info passate dal plugin al wizard
								//GeneralUtility::debug($this->P);
								$this->P=$var;
								//GeneralUtility::debug($var);
								...
								if (!$connection_obj) {
									...
									$content.=		'<a href="#" onclick="'.htmlspecialchars('jumpToUrl(unescape(\''.rawurlencode($this->P['returnUrl']).'\')); return false;').'">'.
													'<img src="'.$icon['closedok']['uri'].'" width="18" height="16" class="c-inputButton" title="Close document" />'.
													'</a>&nbsp;';
									$content.=		'<input type="image" src="'.$icon['refresh']['uri'].'" class="c-inputButton" name="_refresh" title="refresh document" width="16" height="16" />&nbsp;';
									...
									$content.= '</form>';
										...
										$content.='<hr style="background:#bbb;height:1px;">';
...
/*
// Make instance:
$SOBE = GeneralUtility::makeInstance('WfqbeSelectWizard');
$SOBE->init();
