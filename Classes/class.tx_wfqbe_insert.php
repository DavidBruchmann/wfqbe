...
use \Barlian\Wfqbe\Utility\StringUtility;
...
class tx_wfqbe_insert {
	...
	function main($conf, $cObj, $pibase) {
		...
		if (($this->pibase->piVars['wfqbe_editing_mode']==1
			&& $this->pibase->piVars['wfqbe_add_new']=="")
			|| ($this->pibase->piVars['wfqbe_edit_subrecord']>0 && $this->pibase->piVars['wfqbe_add_new']!="")
		) {
			$this->mode = 'edit';
		} elseif (
			($this->pibase->piVars['wfqbe_deleting_mode']==1 && $this->pibase->piVars['wfqbe_add_new']=="")
			|| ($this->pibase->piVars['wfqbe_delete_subrecord']>0 && $this->pibase->piVars['wfqbe_add_new']!="")
		) {
			$this->mode = 'delete';
		} else {
			$this->mode = 'insert';
	...
	function do_sGetFormResult($row, $h, $new_id='') {
		...
		if (ExtensionManagementUtility::isLoaded('rtehtmlarea')) {
			...
			require_once(ExtensionManagementUtility::extPath('rtehtmlarea').'pi2/class.tx_rtehtmlarea_pi2.php');
			if (!$this->RTEObj) $this->RTEObj = GeneralUtility::makeInstance('tx_rtehtmlarea_pi2');
		...
		*/
		...
		if (is_array($new_id)) {
			if ($new_id[2]=='unset') {
				if (   $this->blocks['fields'][$new_id[0]]['form']['multiple']=='si'
					|| $this->blocks['fields'][$new_id[0]]['form']['multiple']=='1'
					|| $this->blocks['fields'][$new_id[0]]['type']=='select'
				) {
		...
		if (isset($this->pibase->piVars['submit_confirm'])
			&& !isset($this->pibase->piVars['submit_insert'])
			&& !isset($this->pibase->piVars['submit_modify'])
		) {
			// Show the confirmation page
			$content .= $this->showPageConfirmation($this->pibase->piVars, $this->blocks, $h, $file, $row);
		} elseif ($this->pibase->piVars['wfqbe_deleting_mode']==1
			&& !isset($this->pibase->piVars['submit_insert'])
			&& !isset($this->pibase->piVars['submit_modify'])
		) {
		...
		} elseif (isset($this->pibase->piVars['submit_insert'])
			&& $this->pibase->piVars['submit_insert']!='wfqbe_no'
		) {
			...
			if ($results['inserted']==1 && $this->conf['email.']['send_email']==1 && $this->conf['email.']['notify_email']!='') {
				$mail = GeneralUtility::makeInstance('tx_wfqbe_mail');
			...
			if ($results['inserted']==1 && $this->conf['email.']['send_email_user']==1 && $this->conf['email.']['field_email_user']!='' && GeneralUtility::validEmail($this->pibase->piVars[$this->conf['email.']['field_email_user']])) {
				$mail = GeneralUtility::makeInstance('tx_wfqbe_mail');
				$mail->init($this->cObj, $this->conf, $this->pibase->piVars, $this);
			...
			if ($this->conf['email.']['debug']==1) {
				...
				DebugUtility::debug($this->pibase->piVars);
			...
			if ($this->conf['ff_data']['clear_cache']!='') {
				...
				if (is_array($clear_cache) && count($clear_cache)>0) {
					...
					foreach ($clear_cache as $pUid) {
						if (MathUtility::canBeInterpretedAsInteger($pUid)) {
			...
			if (!$results['inserted']) {
				...
				if ($this->pibase->piVars['wfqbe_add_new']!='') {
					...
					$query = '';
					if ($this->conf['ff_data']['redirectQuery']!='') {
						if (is_array($results['insert_data'])) {
					...
					header('Location: '.GeneralUtility::getIndpEnv('TYPO3_SITE_URL').$action);
				} else {
			...
			&& $this->pibase->piVars['wfqbe_add_new']!=''
			&& $this->pibase->piVars['wfqbe_delete_subrecord']!=''
			&& !isset($this->pibase->piVars['submit_insert'])
			&& !isset($this->pibase->piVars['submit_modify'])
		) {
			// Show the confirmation page
	...
	function sGetInsertForm_Ajax() {
		...
		if ($this->pibase->piVars['wfqbe_this_query']!="") {
			...
			$CONN = GeneralUtility::makeInstance("tx_wfqbe_connect");
	...
	function showInsertModule($content, $blocks, $h, $row, $required=array()) {
		...
		if (ExtensionManagementUtility::isLoaded('xajax') && $this->conf['enableXAJAX']==1) {
			$blockList .= '<input type="hidden" value="' . $row->getUid() . '" name="tx_wfqbe_pi1[wfqbe_this_query]" />';
		...
		if ($this->pibase->piVars['wfqbe_add_new']!='') {
			...
			$API = GeneralUtility::makeInstance("tx_wfqbe_utils");

			$tmpValue = is_array($this->pibase->piVars['orig']) ? '' : 'orig';

			$hiddenFields = $API->getHiddenFields($this->pibase->piVars, $tmpValue, 'submit_modify');
			$hiddenFields .= '<input type="hidden" name="tx_wfqbe_pi1[wfqbe_add_new]" value="'.$this->pibase->piVars['wfqbe_add_new'].'" />';
			$hiddenFields .= '<input type="hidden" name="tx_wfqbe_pi1[wfqbe_edit_subrecord]" value="'.$this->pibase->piVars['wfqbe_edit_subrecord'].'" />';
			$hiddenFields .= '<input type="hidden" name="tx_wfqbe_pi1[wfqbe_delete_subrecord]" value="'.$this->pibase->piVars['wfqbe_delete_subrecord'].'" />';

			$wfqbe_add_new = $this->pibase->piVars['wfqbe_add_new'];
			$this->old_piVars = $this->pibase->piVars;
			if ($this->pibase->piVars['submit_modify']=='') {
				if (MathUtility::canBeInterpretedAsInteger($this->pibase->piVars['wfqbe_edit_subrecord'])
					&& $this->pibase->piVars['wfqbe_edit_subrecord']!=''
				) {
			...
			if ($this->mode == 'edit') {
				if ($this->blocks['ID_field']=='') {
					...
					if ($this->pibase->piVars['wfqbe_id_field']!="" && is_int(intval($this->pibase->piVars['wfqbe_id_field']))) {
						$hiddenFields .= '<input type="hidden" name="tx_wfqbe_pi1[wfqbe_id_field]" value="'.$this->pibase->piVars['wfqbe_id_field'].'" />';
						$editing_record = false;
					} else {
						$hiddenFields .= '<input type="hidden" name="tx_wfqbe_pi1[wfqbe_id_field]" value="'.$this->pibase->piVars[$this->blocks['ID_field']].'" />';
						$editing_record = $this->initValues($h);  // Now I retrieve the values that I want to edit from the table
						unset($this->pibase->piVars[$this->blocks['ID_field']]);
					}
					$hiddenFields .= '<input type="hidden" name="tx_wfqbe_pi1[wfqbe_editing_mode]" value="1" />';
				}
			} elseif ($this->mode == 'delete') {
				if ($this->blocks['ID_field']=='') {
					return "";
					...
					$hiddenFields .= '<input type="hidden" name="tx_wfqbe_pi1[wfqbe_deleting_mode]" value="1" />';
				}
			}
		} else {
			if ($this->mode=='edit') {
		...
		$mA['###INSERT_HIDDEN_FIELDS###'] = $hiddenFields;
		unset($hiddenFields);
		...
		if ($this->mode == 'edit' && $this->blocks['ID_restricting']!='') {
			$res = $this->checkIDRestricting($h, $this->blocks['ID_restricting'], $editing_record);
			if ($res['notAllowed']==1) {
				DebugUtility::debug('not allowed');
				return $res['content'];
			}
		}
		...
		$params['addQueryString'] = 1;
		$mA['###CONF_INSERT###'] = $this->cObj->typoLink_URL($params);

		//$newUrl = $this->pibase->pi_getPageLink($GLOBALS['TSFE']->id);
		//$mA['###CONF_INSERT###'] = \TYPO3\CMS\Core\Utility\GeneralUtility::locationHeaderUrl($newUrl);

		$mA['###CONF_DIVID###'] = $this->conf['ff_data']['div_id'];
		$mA['###INSERT_DESCRIPTION###'] = $row->getDescription();

		if ($this->mode=='edit') {
				$mA['###CONF_SUBMIT_LABEL###'] = 'Update'; //$this->pibase->pi_getLL('update_submit', 'Update');
		} elseif ($this->mode=='delete') {
				$mA['###CONF_SUBMIT_LABEL###'] = 'Delete'; //$this->pibase->pi_getLL('delete_submit', 'Delete');
		} else {
			$mA['###CONF_SUBMIT_LABEL###'] = 'Insert'; //$this->pibase->pi_getLL('insert_submit', 'Insert');
		}
		...
		$mA['###LABEL_REQUIRED###'] = 'Please, complete this field'; //$this->pibase->pi_getLL('required', 'Please, complete this field');
		$mA['###LABEL_CANCEL###'] = 'Cancel'; //$this->pibase->pi_getLL('cancel', 'Cancel');
	...
	function showInsertField($key, $value, $h, $blockTemplate, $hiddenTemplate, $show_required=false, $odd=0, $custom_validation="") {
		...
		switch ($value['type']) {
			...
			case 'calendar':
				...
				break;
			case 'datepicker':
				$rA['###INSERT_FIELD###'] = rs_datepicker::showDate($value, $key, $name);
				break;
			case 'timepicker':
				$rA['###INSERT_FIELD###'] = rs_datepicker::showTime($value, $key, $name);
				break;
			case 'password':
		...
		if ($value['form']['add_new']!='') {
			if (substr($this->conf['insert.']['add_new.']['icon'],0,4)=='EXT:') {
				$path = ExtensionManagementUtility::siteRelPath("wfqbe").substr($this->conf['insert.']['add_new.']['icon'],10);
			} else {
				$path = $this->conf['insert.']['add_new.']['icon'];

				$rA['###INSERT_ADD_NEW###'] = "<a href='#' onclick=\"javascript:submitActions(); document.getElementById('wfqbe_add_new').value='".$key."'; document.getElementById('".$this->conf['ff_data']['div_id']."_form').submit(); return false;\"><img src='".GeneralUtility::getIndpEnv('TYPO3_SITE_URL').$path."' /></a>";
				
				//$rA['###INSERT_ADD_NEW###'] = "<button type='submit' name='tx_wfqbe_pi1[wfqbe_add_new]' value='".$key."'><img src='".GeneralUtility::getIndpEnv('TYPO3_SITE_URL').$path."' /></button>";
				
				//$rA['###INSERT_ADD_NEW###'] = "<input type='image' name='tx_wfqbe_pi1[wfqbe_add_new]' value='".$key."' src='".GeneralUtility::getIndpEnv('TYPO3_SITE_URL').$path."' />";
			}
		} else {
			$rA['###INSERT_ADD_NEW###'] = '';
		}
		...
		if ($value['form']['select_wizard']!='') {
			if (substr($this->conf['insert.']['select_wizard.']['icon'],0,4)=='EXT:') {
				$path = ExtensionManagementUtility::siteRelPath("wfqbe").substr($this->conf['insert.']['select_wizard.']['icon'],10);
			} else {
				$path = $this->conf['insert.']['select_wizard.']['icon'];
				
				$rA['###INSERT_SELECT_WIZARD###'] = "<a href='#' onclick=\"javascript:submitActions(); document.getElementById('wfqbe_select_wizard').value='".$key."'; document.getElementById('".$this->conf['ff_data']['div_id']."_form').submit(); return false;\"><img src='".GeneralUtility::getIndpEnv('TYPO3_SITE_URL').$path."' /></a>";
				
				//$rA['###INSERT_SELECT_WIZARD###'] = '<input id="'.$name.'" type="image" name="tx_wfqbe_pi1[wfqbe_select_wizard]" value="'.$key.'" src="'.GeneralUtility::getIndpEnv('TYPO3_SITE_URL').$path.'" />';
			}
		} else {
	...
	function showDisplay($value, $name, $id) {
		if ($this->pibase->piVars[$name]!='') {
			return StringUtility::charToEntity( $this->pibase->piVars[$name] ) .
				'<input id="' . $id . '" type="hidden" name="tx_wfqbe_pi1[' . $name . ']" value="' . htmlspecialchars( $this->pibase->piVars[$name] ) . '" />';
		}
		if ($value['form']['code']!='') {
	...
	function showInput($value, $name, $id) {
		...
		return '<input'.$attributes.' id="'.$id.'" type="text" name="tx_wfqbe_pi1['.$name.']" value="'.StringUtility::charToEntity($fieldValue).'" />';
	}
	...
	function showCalendar($value, $name, $id, &$blockTemplate) {
		...
		return '<input id="'.$id.'" type="text" name="tx_wfqbe_pi1['.$name.']" value="'.StringUtility::charToEntity($fieldValue).'" />';
			...
			while($array = $ris -> FetchRow()) {
				if ($value['form']['allow_edit']==1) {
					if (substr($this->conf['insert.']['edit_wizard.']['icon'],0,4)=='EXT:') {
						$path = ExtensionManagementUtility::siteRelPath("wfqbe").substr($this->conf['insert.']['edit_wizard.']['icon'],10);
					} else {
						$path = $this->conf['insert.']['edit_wizard.']['icon'];
					}
					$content .= "<a href='#' onclick=\"javascript:submitActions(); document.getElementById('wfqbe_add_new').value='".$name."'; document.getElementById('wfqbe_edit_subrecord').value='".$array[$value['form']['field_insert']]."'; document.getElementById('".$this->conf['ff_data']['div_id']."_form').submit(); return false;\"><img src='".GeneralUtility::getIndpEnv('TYPO3_SITE_URL').$path."' /></a>";
				}
				if ($value['form']['allow_delete']==1) {
					if (substr($this->conf['insert.']['delete_wizard.']['icon'],0,4)=='EXT:') {
						$path = ExtensionManagementUtility::siteRelPath("wfqbe").substr($this->conf['insert.']['delete_wizard.']['icon'],10);
					} else {
	...
	function showTextarea($value, $name, $id) {
		$wfqbe = GeneralUtility::_GP('tx_wfqbe_pi1');
		if ($value['form']['rows']=="") {
			$value['form']['rows'] = 5;
		}
		if ($value['form']['cols']=="") {
		...
		if ($value['form']['rte']==1 && ExtensionManagementUtility::isLoaded('rtehtmlarea')) {
	...
	function substituteInsertMarkers($where) {
		...
		// This function substitutes the markers like ###WFQBE_X### with GeneralUtility::_GP('wfqbe[x]')
		...
		if (is_array($tsMarkers)) {
			foreach ($tsMarkers as $marker) {
				if ($this->conf['customInsert.'][$this->row->getUid().'.'][$marker]!="" && (($markerParametri["###".$marker."###"]=='' && $this->conf['customInsert.'][$this->row->getUid().'.'][$marker."."]["overrideIfEmpty"]==1) || $this->conf['customInsert.'][$this->row->getUid().'.'][$marker."."]["overrideAlways"]==1)) {
					$confArray = $this->conf["customInsert."][$this->row->getUid()."."][$marker."."];
					//$confArray = $this->parseTypoScriptConfiguration($confArray, $wfqbeArray);
					eval('$markerParametri["###".$marker."###"]=$this->cObj->'.$this->conf["customInsert."][$this->row->getUid()."."][$marker].'($confArray);');
				}
			}
		...
		if (sizeof($required['required'])==0 && sizeof($required['custom_validation'])==0) {
			...
			foreach ($data as $key => $value) {
				if (!is_numeric($key) && !$final) {
					$blockList .= "<input type='hidden' name='tx_wfqbe_pi1[".$key."]' value='".StringUtility::charToEntity($value)."' />";
				}
			}
			...
			foreach ($blocks['fields'] as $key => $config) {
				if ($blocks['fields'][$key]['type']!='PHP function' && $blocks['fields'][$key]['type']!='Raw HTML') {
					...
					if (is_array($value) && $value[0]!='') {
						...
						foreach ($value as $i => $v) {
							$mA['###INSERT_FIELD###'] .= "<input type='hidden' name='tx_wfqbe_pi1[".$key."][".$i."]' value='".StringUtility::charToEntity($v)."' />";
						}
					} else {
						...
						$mA['###INSERT_FIELD###'] = "<input type='hidden' name='tx_wfqbe_pi1[".$key."]' value='".StringUtility::charToEntity($value)."' />";
					}
			...
			if ($final && ExtensionManagementUtility::isLoaded('xajax') && $this->conf['enableXAJAX']==1) {
			...
			$params['addQueryString'] = 1;
			$mA['###CONF_INSERT###'] = $this->cObj->typoLink_URL($params);
			$mA['###CONF_SUBMIT_LABEL###'] = 'Insert'; // $this->pibase->pi_getLL('insert_confirm', 'Insert');
			$mA['###LABEL_PLEASE_CONFIRM###'] = 'Please confirm'; // $this->pibase->pi_getLL('please_confirm', 'Please confirm');
			...
			if ($this->pibase->piVars['wfqbe_add_new']!='') {
				...
				if ($this->mode=='edit') {
					$mA['###CONF_SUBMIT_LABEL###'] = 'Update'; // $this->pibase->pi_getLL('update_confirm', 'Update');
				} elseif ($this->mode=='delete') {
					$mA['###CONF_SUBMIT_LABEL###'] = 'Delete'; // $this->pibase->pi_getLL('delete_confirm', 'Delete');
				}
				unset($this->pibase->piVars);
				...
				if ($this->mode=='edit') {
					...
					$mA['###LABEL_COMMIT_RESULT###'] = 'Record updated'; // $this->pibase->pi_getLL('commit_result_'.$this->mode, 'Record updated');

				} elseif ($this->mode=='delete') {
					$mA['###CONF_SUBMIT_LABEL###'] = 'Delete'; // $this->pibase->pi_getLL('delete_confirm', 'Delete');
					$mA['###LABEL_COMMIT_RESULT###'] = 'Record deleted'; // $this->pibase->pi_getLL('commit_result_'.$this->mode, 'Record deleted');
					if ($this->blocks['ID_field']=='') {
						return "";
					} else {
						if ($this->pibase->piVars['wfqbe_id_field']!="" && is_int(intval($this->pibase->piVars['wfqbe_id_field']))) {
					...
					$mA['###CONF_SUBMIT_LABEL###'] = 'Insert'; // $this->pibase->pi_getLL('insert_confirm', 'Insert');
					$mA['###LABEL_COMMIT_RESULT###'] = 'Record inserted'; // $this->pibase->pi_getLL('commit_result_'.$this->mode, 'Record inserted');
				}
			}

			$mA['###INSERT_DESCRIPTION###'] = $row['description'];
			$mA['###LABEL_MODIFY###'] = 'Modify'; // $this->pibase->pi_getLL('modify', 'Modify');
			$mA['###LABEL_NEW_RECORD###'] = 'New record'; // $this->pibase->pi_getLL('new_record', 'New record');
				...
				foreach ($blocks['fields'] as $key => $config) {
					if ($config['form']['when'][$this->mode]==1) {
						if ($config['field']=='wfqbe_custom') {
							...
							&& $config['form']['rte']==1
							&& ExtensionManagementUtility::isLoaded('rtehtmlarea')
						) {
							if ($this->RTEObj->isAvailable()) {
								...
								$insert_data[$blocks['fields'][$key]['field']] = $h->qstr(
									StringUtility::entityToChar(
										$this->RTEObj->transformContent('db',$data[$key], 'tx_wfqbe_pi1', $key, $insert_data, $this->specConf, $this->thisConfig, '', $this->thePidValue)
									)
								);
							...
							if ($blocks['fields'][$key]['type']=="datetype" && $val!="") {
								$val = $this->get_datestamp($val, $blocks['fields'][$key]);
							}
							if ($blocks['fields'][$key]['type']=="datepicker" && $val!="") {
								$val = rs_datepicker::date2unixtime($val);
							}
							if ($blocks['fields'][$key]['type']=="timepicker" && $val!="") {
								$val = rs_datepicker::time2unixtime($val);
							}
							...
							if ($blocks['fields'][$key]['type']=="calendar" && $blocks['fields'][$key]['form']['convert_to_date_oracle']=='si') {
								...
								if ($val=='' && $blocks['fields'][$key]['form']['insert_NULL']==1) {
									...
									$insert_data[$blocks['fields'][$key]['field']] = $h->qstr(StringUtility::entityToChar($val));
								}
	...
	function checkRequired($data, $blocks) {
		...
		foreach ($data as $key => $value) {
			if (!in_array(strtoupper($key), array_keys($blocks['fields']))) {
				...
				if ($blocks['fields'][strtoupper($key)]['type']=="datetype"
					&& $data[$key]!=''
					&& !$this->is_dateValid($data[$key], $blocks['fields'][strtoupper($key)])
				) {
					$required['required'][] = strtoupper($key);
				}

				//	This is neccessary to check if the entered date is correct or not
				if ($blocks['fields'][strtoupper($key)]['type']=="calendar"
					&& $data[$key]!=''
					&& !$this->is_dateValid($data[$key], $blocks['fields'][strtoupper($key)])
				) {
					$required['required'][] = strtoupper($key);
	...
	function initValues($h, $id='') {
		...
		foreach ($this->blocks['fields'] as $key => $config) {
			if ($this->mode=="delete" || $config['form']['when'][$this->mode]==1) {
				...
				} elseif ($config['type']=='datepicker') {
					$this->pibase->piVars[$key] = rs_datepicker::unixtime2date($row[$config['field']]);
				} elseif ($config['type']=='timepicker') {
					$this->pibase->piVars[$key] = rs_datepicker::unixtime2time($row[$config['field']]);
	...
	function checkIDRestricting($h, $idRestrictQuery, $editing_record) {
		$API = GeneralUtility::makeInstance('tx_wfqbe_results');
		...
		if (is_array($tsMarkers)) {
			foreach ($tsMarkers as $marker) {
				...
				if ($this->conf['customRestrictingQuery.'][$this->row['uid'].'.'][$marker]!="" && (($markerParametri["###".$marker."###"]=='' && $this->conf['customRestrictingQuery.'][$this->row['uid'].'.'][$marker."."]["overrideIfEmpty"]==1) || ($markerParametri["###".$marker."###"]!='' && $this->conf['customRestrictingQuery.'][$this->row['uid'].'.'][$marker."."]["overrideIfNotEmpty"]==1) || $this->conf['customRestrictingQuery.'][$this->row['uid'].'.'][$marker."."]["overrideAlways"]==1)) {
					if ($markerParametri["###".$marker."###"]==''
						&& $this->conf['customRestrictingQuery.'][$this->row['uid'].'.'][$marker."."]["overrideIfEmpty"]==1
					) {
						$emptyCase = true;
					}
				...
				} elseif (!$emptyCase && $this->conf['customRestrictingQuery.'][$this->row['uid'].'.'][$marker.'.']!=""
					&& $this->conf['customRestrictingQuery.'][$this->row['uid'].'.'][$marker."."]["wfqbe."]['floatval']==1
				) {
		...
		if (!$ris) {
			if ($this->pibase->beMode==1) {
				global $LANG;
				$content = 'not_allowed_idrestricting'; // $LANG->getLL('not_allowed_idrestricting');
			} else {
				$content = 'not_allowed_idrestricting'; // $this->pibase->pi_getLL('not_allowed_idrestricting');
			}
