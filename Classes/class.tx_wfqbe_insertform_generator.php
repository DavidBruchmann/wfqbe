...
*  Copyright notice
*
*  (c) 2007 Mauro Lorenzutti (Webformat srl) (mauro.lorenzutti@webformat.com)
*  All rights reserved
*
...
//$content .= \TYPO3\CMS\Core\Utility\GeneralUtility::view_array($this->blocks);
		...
		if ($this->blocks['table']!="")	{
			...
			if (is_array($this->blocks['fields']))	{
				foreach($this->blocks['fields'] as $key => $value)	{
					...
					if ($numForm > 0)
						$content.= ' <a href="#" onclick="document.getElementById(\'wfqbe_up\').value='.$numForm.'; updateForm()"><img src="'.\TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('TYPO3_SITE_URL').'typo3/sysext/t3skin/icons/gfx/button_up.gif" /></a>';
					if ($numForm < sizeof($this->blocks['fields'])-1)
						$content.= ' <a href="#" onclick="document.getElementById(\'wfqbe_down\').value='.$numForm.'; updateForm()"><img src="'.\TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('TYPO3_SITE_URL').'typo3/sysext/t3skin/icons/gfx/button_down.gif" /></a>';
					...
					switch ($this->blocks['fields'][$key]['type'])	{
                        ...
                        // EW
                        case 'datepicker':
							$content .= $this->showDate($numForm, $this->blocks['fields'][$key]['form']);
							break;
                        case 'timepicker':
							$content .= $this->showTime($numForm, $this->blocks['fields'][$key]['form']);
							break;
						case 'password':
    ...
    /**
     * EW
	 * Function used to create a time field and/or a combined date/time field with the jquerylib timepicker
	 * 
	 * @param 	[int]			$key: field index (used to sort the fields)
	 * @param 	[array]			$form: field configuration
	 * @return 	[string]		html form
	 */
	function showTime($key, $form)	{
		$html = $this->labelInput($key, $form['label']);
        
        $html.='<br /><br />---------------- end of new field - RS -------------------------------<br />';
	...
	function showTextarea($key, $form)	{
		...
		if ($form['rte']==1)
			...
			$checked = '';
		$html .= '<br />Show RTE: <input '.$checked.'type="checkbox" name="wfqbe[fields]['.$key.'][form][rte]" value="1" />';
