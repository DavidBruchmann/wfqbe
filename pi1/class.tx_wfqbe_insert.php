...
//t3lib_div::debug(array('mode'=>$this->mode, 'query'=>$this->blocks['ID_restricting'], 'res'=>$res, 'piVars'=>$this->pibase->piVars, 'editingrecord'=>$editing_record));
			...
			if ($results['inserted']==1 && $this->conf['email.']['send_email']==1 && $this->conf['email.']['notify_email']!='')	{
				require_once(t3lib_extMgm::extPath('wfqbe').'lib/class.tx_wfqbe_mail.php');
				$mail = t3lib_div::makeInstance('tx_wfqbe_mail');
			...
			if ($results['inserted']==1 && $this->conf['email.']['send_email_user']==1 && $this->conf['email.']['field_email_user']!='' && t3lib_div::validEmail($this->pibase->piVars[$this->conf['email.']['field_email_user']]))	{
				require_once(t3lib_extMgm::extPath('wfqbe').'lib/class.tx_wfqbe_mail.php');
				$mail = t3lib_div::makeInstance('tx_wfqbe_mail');
				$mail->init($this->cObj, $this->conf, $this->pibase->piVars, $this);
			...
			if (!$results['inserted'])	{
	        	...
	        	elseif ($results['id']=='-2' && $this->conf['ff_data']['deletedPage']!='')	{
	        		...
	        		header('Location: '.t3lib_div::getIndpEnv('TYPO3_SITE_URL').$action);
	        	}	else	{
...
t3lib_div::debug('not allowed');
		...
		$mA['###CONF_INSERT###'] = t3lib_div::locationHeaderUrl($this->pibase->pi_getPageLink($GLOBALS['TSFE']->id));
	...
	function showInsertField($key, $value, $h, $blockTemplate, $hiddenTemplate, $show_required=false, $odd=0, $custom_validation="")	{
		...
		if ($value['form']['add_new']!='')	{
			if (substr($this->conf['insert.']['add_new.']['icon'],0,4)=='EXT:')
				$path = t3lib_extMgm::siteRelPath("wfqbe").substr($this->conf['insert.']['add_new.']['icon'],10);
			else
				$path = $this->conf['insert.']['add_new.']['icon'];
$rA['###INSERT_ADD_NEW###'] = "<a href='#' onclick=\"javascript:submitActions(); document.getElementById('wfqbe_add_new').value='".$key."'; document.getElementById('".$this->conf['ff_data']['div_id']."_form').submit(); return false;\"><img src='".t3lib_div::getIndpEnv('TYPO3_SITE_URL').$path."' /></a>";
//$rA['###INSERT_ADD_NEW###'] = "<button type='submit' name='tx_wfqbe_pi1[wfqbe_add_new]' value='".$key."'><img src='".t3lib_div::getIndpEnv('TYPO3_SITE_URL').$path."' /></button>";
			//$rA['###INSERT_ADD_NEW###'] = "<input type='image' name='tx_wfqbe_pi1[wfqbe_add_new]' value='".$key."' src='".t3lib_div::getIndpEnv('TYPO3_SITE_URL').$path."' />";
		}	else	{
			$rA['###INSERT_ADD_NEW###'] = '';
		}
		...
		if ($value['form']['select_wizard']!='')	{
			if (substr($this->conf['insert.']['select_wizard.']['icon'],0,4)=='EXT:')
				$path = t3lib_extMgm::siteRelPath("wfqbe").substr($this->conf['insert.']['select_wizard.']['icon'],10);
			else
				$path = $this->conf['insert.']['select_wizard.']['icon'];
$rA['###INSERT_SELECT_WIZARD###'] = "<a href='#' onclick=\"javascript:submitActions(); document.getElementById('wfqbe_select_wizard').value='".$key."'; document.getElementById('".$this->conf['ff_data']['div_id']."_form').submit(); return false;\"><img src='".t3lib_div::getIndpEnv('TYPO3_SITE_URL').$path."' /></a>";
			//$rA['###INSERT_SELECT_WIZARD###'] = '<input id="'.$name.'" type="image" name="tx_wfqbe_pi1[wfqbe_select_wizard]" value="'.$key.'" src="'.t3lib_div::getIndpEnv('TYPO3_SITE_URL').$path.'" />';
		}	else	{
		...
		if (is_array($tsMarkers))	{
			foreach ($tsMarkers as $marker)	{
				if ($this->conf['customInsert.'][$this->row['uid'].'.'][$marker]!="" && (($markerParametri["###".$marker."###"]=='' && $this->conf['customInsert.'][$this->row['uid'].'.'][$marker."."]["overrideIfEmpty"]==1) || $this->conf['customInsert.'][$this->row['uid'].'.'][$marker."."]["overrideAlways"]==1))	{
					...
					eval('$markerParametri["###".$marker."###"]=$this->cObj->'.$this->conf["customInsert."][$this->row["uid"]."."][$marker].'($confArray);');
				}
			}
...
//t3lib_div::debug($_FILES);
	...
	function showPageConfirmation($data, $blocks, $h, $file, $rowQuery, $final=false)	{
		...
		if (sizeof($required['required'])==0 && sizeof($required['custom_validation'])==0)	{
			...
			if ($final && t3lib_extMgm::isLoaded('xajax') && $this->conf['enableXAJAX']==1)
				...
				foreach ($blocks['fields'] as $key => $config)	{
					if ($config['form']['when'][$this->mode]==1)	{
						...
						elseif ($config['type']=='PHP function')	{
							...
							continue;
						}	elseif ($config['type']=='textarea' && $config['form']['rte']==1 && t3lib_extMgm::isLoaded('rtehtmlarea'))	{
							if($this->RTEObj->isAvailable()) {
...
//t3lib_div::debug($this->pibase->piVars);
	...
	function parseDate($date, $format)	{
		...
		if (t3lib_div::inArray($calendarDateTimeFormats, $format))	{
			$temp = explode(' ', $date);
