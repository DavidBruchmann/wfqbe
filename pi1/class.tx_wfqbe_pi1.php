...
require_once(t3lib_extMgm::extPath('wfqbe')."pi1/class.tx_wfqbe_search.php");
require_once(t3lib_extMgm::extPath('wfqbe')."pi1/class.tx_wfqbe_insert.php");
...
class tx_wfqbe_pi1 extends tslib_pibase {
	...
	function main($content,$conf)	{
		...
		if (!t3lib_div::_GP('tx_wfqbe_pi1') && $form_built) {
	...
	function initFF()	{
		...
		/*if (strpos($this->conf['template'], t3lib_div::getIndpEnv('TYPO3_DOCUMENT_ROOT'))!==false)
			$this->conf['template'] = str_replace(t3lib_div::getIndpEnv('TYPO3_DOCUMENT_ROOT').'/', '', $this->conf['template']);*/
	}
	...
	function processSearch($data)	{
		...
		$SEARCH = t3lib_div::makeInstance("tx_wfqbe_search");
	...
	function processInsert($data)	{
		...
		$INSERT = t3lib_div::makeInstance("tx_wfqbe_insert");
	...
	function do_sGetFormResult($row, $h)	{
		if ($row['type']=="insert")	{
			...
			// SELECT
			$SELECT = t3lib_div::makeInstance("tx_wfqbe_results");
