...
class tx_wfqbe_search {
	...
	function do_sGetForm($row, $h, &$form_built)	{
		...
		if (t3lib_extMgm::isLoaded('xajax') && $this->conf['enableXAJAX']==1 && ($this->conf['ff_data']['resultsPage']==$GLOBALS['TSFE']->id || $this->conf['ff_data']['resultsPage']==''))	{
...
//t3lib_div::debug($blocks);
		...
		$blockList = '';
		if (t3lib_extMgm::isLoaded('xajax') && $this->conf['enableXAJAX']==1 && $this->conf['ff_data']['resultsPage']==$GLOBALS['TSFE']->id)
	...
	function getSearchField($key, $value, $h, $blockTemplate, $odd=0, $row)	{
		...
		if ($name!='' && t3lib_div::inList($this->conf["customSearch."][$row['uid']."."]['excludeFields'], $name))	{
			return '';
		}
			...
			else
			{
				...
				foreach ($value['form'] as $f => $v)	{
					if ($this->conf["customQuery."][$this->row['uid']."."][$f]!="")	{
						...
						eval('$value[\'form\'][$f]=$this->cObj->'.$this->conf["customQuery."][$this->row["uid"]."."][$f].'($confArray);');
					}
				}
...
//t3lib_div::debug($query);
			...
			if ($ris!==false)	{
				while($array = $ris -> FetchRow()){
					...
					if ($this->conf["customProcess."][$this->row['uid']."."][$value['form']['field_view']]!="")	{
						...
						$confArray = $this->parseTypoScriptConfiguration($confArray, $wfqbeArray);
						eval('$array[0]=$this->cObj->'.$this->conf["customProcess."][$this->row["uid"]."."][$value['form']['field_view']].'($confArray);');
					}	else	{
						$array[0]=$array[0];
					}
	 ...
	 * This function substitutes the ###WFQBE_VARIABLE_NAME### markers with t3lib_div::_GP('wfqbe[variable_name]') 
	 */
