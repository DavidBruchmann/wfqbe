...
$LANG->includeLLFile('EXT:wfqbe/tx_wfqbe_query_search/locallang.xml');
...
class tx_wfqbe_searchform_generator{
	...
	* @return	[string]	$content: stringa che contiene l'html del form	
	...
	function renderField($key, $value, $h)	{
		...
		if ($numForm > 0)
			$content.= ' <a href="#" title="Move up ('.($numForm).')" onclick="document.getElementById(\'wfqbe_up\').value='.($numForm).'; updateForm()"><img src="'.t3lib_div::getIndpEnv('TYPO3_SITE_URL').'typo3/sysext/t3skin/icons/gfx/button_up.gif" /></a>';
		if ($numForm < sizeof($this->blocks['fields'])-2)
			$content.= ' <a href="#" title="Move down ('.($numForm).')" onclick="document.getElementById(\'wfqbe_down\').value='.($numForm).'; updateForm()"><img src="'.t3lib_div::getIndpEnv('TYPO3_SITE_URL').'typo3/sysext/t3skin/icons/gfx/button_down.gif" /></a>';
